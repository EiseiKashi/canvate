...
function updateImage(img, maxWidth, maxHeight){
    ...
    var left       = Math.floor((maxWidth-width)/2); 
    var top        = Math.floor((maxHeight-height)/2);
...
function updateThumbImage(event){
    var img = event.currentTarget;
    updateImage(img, thumbWidth, thumbHeight);
    img.style.opacity = 1;
}
...
function createCategory(name, index){
    ...
    var h2      = new HTMLelement('h2',      {id:"CategoryTitle"+index, contenteditable:"true"}, decodeURIComponent(name));
    div3.addChild(h2);
    div2.addChild(div3);
    div1.addChild(div2);
...
function createItem(data, listIndex, itemIndex, isLast){
    var title1 = decodeURIComponent(data[_title1]);
    var last   = isLast ? " last" : "";
    var liItem = new HTMLelement('li', {class:"column-3 clearfix grid-item" + last, listIndex:listIndex, itemIndex:itemIndex}).getHTML();
        liItem.style.width  = thumbWidth  + "px";
        liItem.style.height = thumbHeight + "px";
    var imgCont              = new HTMLelement('div', {class:"imgCont"}).getHTML();
        imgCont.style.width  = thumbWidth  + "px";
        imgCont.style.height = thumbHeight + "px";
        
    var img    = new HTMLelement('img', { 
                                         class:'grid-thumb'
                                         ,alt:title1}).getHTML();
    img.onload = updateThumbImage;
    imgCont.appendChild(img);
    img.src = decodeURIComponent(data[_pic1]);
    
    var h3     = new HTMLelement('h3',   null, decodeURIComponent(title1)).getHTML();
    var p      = new HTMLelement('p',    null, decodeURIComponent(data[_desc1])).getHTML();
    var span1  = new HTMLelement('span', {class:'price1'},decodeURIComponent(data[_price1])).getHTML();
    var span2  = new HTMLelement('span', {class:'price'}, decodeURIComponent(data[_price2])).getHTML();

    liItem.appendChild(imgCont);
    liItem.appendChild(h3);
...
function ItemList(data, listIndex){
    var totalColumns = 4;
    var category     = createCategory(decodeURIComponent(data.name), listIndex).getHTML();
    ...
    for(var index=0; index < length; index++, column++){
        ...
        item   = createItem(data, listIndex, index, isLast);
...
function BigItem(){
    ...
    var bigFile      = document.getElementById("BIG-FILE");
    var uploadButton = document.getElementById("BIG-UPLOAD");
    ...
    this.display = function(data, listIndex, itemIndex){
        ...
        var pic1   = decodeURIComponent(data[_pic1]);
        var name   = decodeURIComponent(data[_title1]);
        var desc   = decodeURIComponent(data[_desc1]);
        var price1 = decodeURIComponent(data[_price1]);
        var price2 = decodeURIComponent(data[_price2]);
        var pic1   = decodeURIComponent(data[_pic1]);
...
function createProductList(senbuList){
    ...
    for(var index=0; index < length; index++){
        ...
        a  = new HTMLelement("a", null, decodeURIComponent(itemList.name));
