...
function CanvateWebGL(element) {
    ...
    window.hexToRGB = function(hex, alpha){
        ...
        hex                 = null == hex ? "#000000" : hex;
        hex                 = hex.toLowerCase();
        hex                 = hex in COLOR_NAMES ? COLOR_NAMES[hex] : hex;
    ...
    var CANVAS          = "canvas";
    var WEB_GL_NAMES    = ["webgl2", "webgl", "experimental-webgl"];
    var IMG             = "img";
    ...
    var Canvate = function (image){
       ...
       this.debug = function(){
        ...
        /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */
        /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */
        // START Canvate Render
        
        this.render = function(mouseX, mouseY, isMasking){
            if(_background != _self.background){
                _background = _self.background;
                var colorArgs = hexToRGB(_background);
                _mainContextOff.clearColor.apply(_mainContextOff, colorArgs);
                _mainContextOff.clear(_mainContextOff.COLOR_BUFFER_BIT);s
            }
    ...
    var initialize = function(){
        ...
        for (var index = 0; index < length; ++index) {
            try {
                ...
                _context        = _mainCanvas.getContext(WEB_GL, {
                    debug:true
                    /*
        ...
        if(null == _mainContextOff){
            throw new Error("The context fails but the WEB GL is supported.");
            // Early return
            return;    
        }
        ...
        _mainCanvas.onmousemove = function(event) {
            ...
            _lastX = ((_mouseX - _bounds.left) - _mainCanvas.width/2)/(_mainCanvas.width/2);    
            _lastY = (_mainCanvas.height/2 - (_mouseY - _bounds.top))/(_mainCanvas.height/2);
        ...
        _stage                  = new Canvate(_mainCanvas);
        _stage.name             = STAGE;
